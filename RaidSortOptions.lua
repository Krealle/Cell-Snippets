...
-- Whether to sort each group individually
local sortEachGroup = true
...
local VALID_SPECROLES = { ["RANGED"] = true, ["MELEE"] = true, ["DAMAGER"] = true, ["HEALER"] = true, ["TANK"] = true }
...
sortRaidFrames = function(layout, which)
	...
	if
		not INTERNAL_SORT_OPTIONS
		or not INTERNAL_SPEC_PRIORITY
		or not INTERNAL_NAME_PRIORITY
		or not INTERNAL_SORT_DIRECTION
	then
...
updateRaidFrames = function(SORTED_RAID_GROUP)
	...
	local header
	if sortEachGroup == true then
		-- iterate over all headers within the raid group
		for i = 1, 8 do
			header = _G["CellRaidFrameHeader" .. i]
			for j = 1, #SORTED_RAID_GROUP do
				---@type Player
				local player = SORTED_RAID_GROUP[j]
...
playerSort = function(playerA, playerB)
	...
	if not playerA.name and not playerB.name then
		return nil
	end
...
roleSort = function(playerA, playerB)
	...
	if not aRole and not bRole then
		return nil
	end
...
specRoleSort = function(playerA, playerB)
	...
	if not aSpecRole and not bSpecRole then
		return nil
	end
...
getPlayerInfo = function(unit)
	...
	if isValidPlayerInfo(cachedInfo) then
		...
		DevAdd(
			{ name, realm, rank, subGroup, level, class, fileName, zone, online, isDead, role, isML, combatRole },
			unit
		)
...
isValidPlayerInfo = function(info)
	return info and info.specId and info.specRole and info.role and info.name and info.realm
end
